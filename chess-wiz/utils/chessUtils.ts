...
export async function getOverview(moves: string) : Promise<string> {
    ...
    const gptRes = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
            model: "gpt-3.5-turbo",
            messages: [
                { role: "system", content: "You are a chess analyst summarizing what happened in this game, where each player went wrong, and how each player could improve" },
                { role: "user", content: `Analyze the following chess game:\n${moves}` },
            ],
            temperature: 0.7,
            max_tokens: 1000,
        }),
